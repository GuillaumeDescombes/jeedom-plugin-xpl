  ...
  public static function parserMessage($_message) {
    ...
    if (is_object($xPL)) {
      ...
      foreach ($list_cmd as $cmd) {
        ...
        if ($shema==$shema_compare && ($type==xPLMessage::xpltrig && $type_compare=='XPL-TRIG' || $type==xPLMessage::xplstat && $type_compare=='XPL-STAT') &&  $adco === $adco_compare) {
          $event_info = array();
...
*/
            ...
            //Adding PTEC
            case "ptec":
              $event_info['value'] =  $_message->bodyItem('ptec'); //<Période tarifaire actuelle>
              break;
  			    case "isousc": 
